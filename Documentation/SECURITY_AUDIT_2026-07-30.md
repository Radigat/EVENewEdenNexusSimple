...
| P0 before first push | SEC-OPEN-03 | Create a deliberate first Git baseline only after owner approval. Install and run Gitleaks (including history once it exists), inspect ignore rules, and stage selectively. At present every project file is untracked, so there is no recoverable repository baseline. |
